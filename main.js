...
function ts(e)
{
    ...
    lastx=e.touches[0].clintX-canvas.offsetLeft;
    lasty=e.touches[0].clintY-canvas.offsetTop;
}
canvas.addEventListener("touchmove",tm);
function tm(e)
{
    current_x=e.touches[0].clintX-canvas.offsetLeft;
    current_y=e.touches[0].clintY-canvas.offsetTop;
   
        ctx.beginPath();
        ctx.strokeStyle=color;
        ctx.lineWidth=width;
        ctx.movetTo(lastx,lasty);
